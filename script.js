    ...
    $('#submit').on('click', function () {
        ...
        $.ajax({
            ...
            .done(function (response) {
                ...
                $("#humidity").html("Le taux d'humidité est de :" + " " + data4 + " " + "%");
                let data5 = JSON.parse(response.wind.speed);
                $("#wind").html("La vitesse du vent est de : " + data5 + "km/h");
                let data6 = JSON.parse(response.main.pressure);
                $("#pressure").html("La pression est de : " + data6 + "hPa");

                let data7 = JSON.parse(response.coord.lat);
                let data8 = JSON.parse(response.coord.lon);
                $("#coord").html("Latitude ="+ " " + data7 + " ° " + " / " + " Longitude ="+ " " + data8 + " ° ");
                let data9 = JSON.parse(response.wind.gust); 
                $("#gust").html("Rafal de vent à : " + data9 + "km/h");
                ...
                function nuage() {
                    ...
                    if (currentTime >= 6 && currentTime <= 18) {
                        ...
                        if (nuage >= 0 && nuage <= 20) {
                            ...
                            $("#clouds").html("La couverture nuageuse est de : " + nuage + " %");
                            $("body").css("background-image", "url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTf095I1bn1gkWcdeWfDJTSwlP8Y5OlQTqUEQ&usqp=CAU'");
                        }
                    }
                }
                nuage();
